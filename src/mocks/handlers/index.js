...
export const handlers = [
  ...
  rest.get('/groups/:id', (req, res, ctx) => {
    if (req.params.id) {
      const matchingStudents = students.filter((student) => student.group.toLowerCase() === req.params.id.toLowerCase());
      return res(
        ctx.status(200),
        ctx.json({
  ...
  rest.get('/students/:id', (req, res, ctx) => {
    if (req.params.id) {
      const matchingStudent = students.find((student) => student.id.toLowerCase() === req.params.id.toLowerCase());
      if (!matchingStudent) {
        return res(
          ctx.status(404),
          ctx.json({
